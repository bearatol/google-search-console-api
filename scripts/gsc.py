...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise GSCError(f"Файл не найден: {path}") from exc
    except json.JSONDecodeError as exc:
        raise GSCError(f"Некорректный JSON: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise GSCError(f"Ожидался JSON-объект: {path}")
    return value
    ...
    *,
    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            raise GSCError(f"Ошибка сети: {exc.reason}") from exc
    raise GSCError("Google API не ответил после повторных попыток")
...
def refresh_access_token(token: dict[str, Any]) -> dict[str, Any]:
    ...
    if not refresh_token:
        raise GSCError("Нет refresh_token. Повторите: python3 scripts/gsc.py auth")
...
def access_token() -> str:
    ...
    if not value:
        raise GSCError("В token.json отсутствует access_token")
    return str(value)
...
class OAuthCallback(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if state != self.expected_state:
            ...
            status = 400
            message = "OAuth state mismatch. Вернитесь в терминал."
        elif query.get("error"):
            type(self).result = {"error": query["error"][0]}
            status = 400
            message = "Авторизация отклонена. Вернитесь в терминал."
        else:
            type(self).result = {"code": query.get("code", [""])[0]}
            status = 200
            message = "Авторизация завершена. Эту вкладку можно закрыть."
...
def oauth_authorize(scope_name: str, no_browser: bool, timeout: int) -> None:
    ...
    with socketserver.TCPServer(("127.0.0.1", 0), OAuthCallback) as server:
        ...
        print(auth_url)
        print(f"\nОжидание callback до {timeout} секунд...")
        if not no_browser:
    ...
    if not OAuthCallback.event.is_set():
        raise GSCError("Истекло время ожидания OAuth callback")
    ...
    if not code:
        raise GSCError("Google не вернул authorization code")
    ...
    save_secret_json(TOKEN_FILE, token)
    print(f"OAuth настроен. Scope: {scope_name}. Токен: {TOKEN_FILE}")
...
def resolve_site(value: str, no_cache: bool = False) -> str:
    ...
    if len(matches) == 1:
        return matches[0]
    if not matches:
        raise GSCError(f"Ресурс для '{value}' не найден. Выполните команду sites.")
    raise GSCError(f"Для '{value}' найдено несколько ресурсов: {', '.join(matches)}")
...
def default_dates(date_from: str | None, date_to: str | None) -> tuple[str, str]:
    ...
    if start > end:
        raise GSCError("date-from не может быть позже date-to")
...
def parse_filters(values: list[str]) -> list[dict[str, Any]]:
    ...
    for value in values:
        ...
        if len(parts) != 3 or not all(parts):
            raise GSCError(
                f"Некорректный filter '{value}'. Формат: dimension:operator:expression"
            )
        filters.append(
...
def print_file_head(path: Path, limit: int = MAX_DISPLAY_ROWS) -> None:
    ...
    if total > limit:
        print(f"... ещё {total - limit} строк")
    print(f"Полные данные: {path}")
...
def command_performance(args: argparse.Namespace) -> None:
    ...
    print(f"Период: {body['startDate']} — {body['endDate']}; строк: {rows}")
...
def command_sitemaps(args: argparse.Namespace) -> None:
    ...
    if args.action == "submit":
        if not args.sitemap_url or not args.confirm:
            raise GSCError("Для submit нужны --sitemap-url и --confirm")
        ...
        print(f"Sitemap отправлен: {args.sitemap_url}")
        return
...
def add_site_argument(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--site", required=True, help="Property URL, sc-domain или домен")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    token_info = commands.add_parser("token-info", help="Показать состояние токена")
    token_info.set_defaults(handler=command_token_info)

    sites = commands.add_parser("sites", help="Список ресурсов")
    sites.add_argument("--no-cache", action="store_true")
    sites.set_defaults(handler=command_sites)

    summary = commands.add_parser("summary", help="SEO-сводка")
